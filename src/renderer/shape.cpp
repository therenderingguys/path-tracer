...
Shape::Shape(void)
    : modelMatrix(1.0f), // Identity
      vertexAttributes(nullptr), boundingBox(nullptr), vertsToDraw(0),
      stride(0) {}
...
void Shape::render(int positionHandle, int modelHandle, int normalHandle) {
  ...
  glVertexAttribPointer(positionHandle, 3, GL_FLOAT, GL_FALSE, stride,
                        vertexAttributes);
  glVertexAttribPointer(normalHandle, 3, GL_FLOAT, GL_FALSE, stride,
                        vertexAttributes + 3);
...
void Shape::addTriangle(float *const vertArray, int &index,
                        ...
                        const glm::vec3 &vertex3, const glm::vec3 &normal) {
  // First Triangle
  // Top Left
...
void Shape::addQuad(float *const vertArray, int &index,
                    const glm::vec3 &vertex1, const glm::vec3 &vertex2,
                    const glm::vec3 &vertex3, const glm::vec3 &vertex4,
