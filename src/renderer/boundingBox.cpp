...
BoundingBox::BoundingBox(const glm::vec3 &dimensions, const glm::vec3 &position,
                         glm::mat4 *matModel)
  ...
  glm::vec3 halfDims(halfDimX, -halfDimY, halfDimZ);
...
bool BoundingBox::contains(const BoundingBox &o) const {
  ...
  return myTL.x < oTL.x && myTL.y > oTL.y && myTL.z < oTL.z && myBR.x > oBR.x &&
         myBR.y < oBR.y && myBR.z > oBR.z;
}
